...
jobjectArray getDetResult(JNIEnv* env, RecPtr faceRecognizer,
  ...
  for (int i = 0; i < size; i++) {
    ...
    int* aligned_image = faceRecognizer->getAlignedImages()[i];
    LOG(INFO) << "Face size " << sizeof(aligned_image)/sizeof(aligned_image[0]);;
    jintArray rgbImageArray = env -> NewIntArray(224*224*3);
    env->SetIntArrayRegion(rgbImageArray, 0, 224*224*3, aligned_image);
    g_pJNI_VisionDetRet->addAlignedImage(env, jDetRet, rgbImageArray);
  }
  return jDetRetArray;
}
...
JNIEXPORT jobjectArray JNICALL
  ...
  LOG(INFO) << "Converting colors";
  cv::cvtColor(rgbaMat, bgrMat, cv::COLOR_RGBA2BGR);
  LOG(INFO) << "Getting detection model";
